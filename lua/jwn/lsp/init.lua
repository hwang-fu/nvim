...
-- `config = function() ... end` in lua/jwn/plugins/spec/treesitter.lua, where it
...
-- Exported for the :checkhealth jwn report (lua/jwn/health.lua),
...
-- ----------------------------------------------------------------------------
-- Public entrypoint. Called from lua/jwn/init.lua.
-- ----------------------------------------------------------------------------
function M.setup()
    setup_filetypes()
    setup_diagnostic_ui()
    ...
    for _, name in ipairs(SERVERS) do
        require("jwn.lsp.servers." .. name).setup()
    end
