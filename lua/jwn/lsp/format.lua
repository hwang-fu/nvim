...
local function setup_format_on_save()
    ...
    vim.api.nvim_create_autocmd("BufWritePre", {
        ...
        callback = function(args)
            ...
            if not vim.b[args.buf].jwn_no_mix_warned then
                vim.b[args.buf].jwn_no_mix_warned = true
                vim.notify(
    ...
    --           typescript via the LispIndent augroup in jwn/init.lua.
...
-- :checkhealth jwn report (lua/jwn/health.lua) reads the same
...
local function check_formatter_binaries()
    ...
    -- startup; the rest live in :checkhealth jwn): every content
