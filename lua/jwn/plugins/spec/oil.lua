...
-- fixed 35-column left split via lua/jwn/explorer.lua, restoring
...
-- / close it) lives in lua/jwn/keymappings/navigation.lua with the other global
	...
	config = function()
		require("oil").setup({
			...
			keymaps = {
				...
				-- lua/jwn/explorer.lua (see the Navigation
				-- comment above for what each does where).
				["<CR>"] = {
					callback = function()
						require("jwn.explorer").select()
					end,
					desc = "Open entry (sidebar-aware)",
				},
				["<C-t>"] = {
					callback = function()
						require("jwn.explorer").smart_close()
					end,
