...
-- behavior - that lives in the other lua/jwn/* modules (init for
...
--   * lua/jwn/plugins/init.lua  - THIS file: bootstrap + setup().
--   * lua/jwn/plugins/spec/<name>.lua - one file per plugin (or per
--     inseparable group: colorschemes.lua, lisp.lua), returning its
--     lazy.nvim spec. Imported automatically via
--     { import = "jwn.plugins.spec" } below. Each plugin carries its
--     keymaps and documentation with it - the keys-live-with-their-
--     plugin convention survives the split.
--
-- Add a new plugin -> create lua/jwn/plugins/spec/<name>.lua
...
-- in lua/jwn/keymappings/editor.lua delegates to `gc` (the old
...
function M.setup()
	...
	require("lazy").setup({
		spec = {
			-- Every module under lua/jwn/plugins/spec/ is imported
			-- automatically; each returns one plugin's spec (or a group).
			{ import = "jwn.plugins.spec" },
		},
	})
end
