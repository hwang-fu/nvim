...
-- in lua/jwa/plugins/spec/rustaceanvim.lua (NOT from lsp/init.lua's SERVERS loop). The
...
--     mason/codelldb, lua/jwa/dap.lua) was removed 2026-08-15 at the
            ...
            default_settings = {
                ["rust-analyzer"] = {
                    ...
                    inlayHints = {
                        ...
                        -- move(&count) shared borrow, move(&mut count)
                        -- mutable borrow, bare name = by value. The
                        -- move(...) label is rust-analyzer's own and is
                        -- technically exact: every capture moves SOMETHING
                        -- into the closure struct - for borrows, the
                        -- reference itself - mirroring Rust's proposed
                        -- capture-clause syntax. (A client-side relabel to
                        -- capture(...) was tried and removed the same day:
                        -- it did not reach the real rendering path, and
                        -- the user dropped the idea.)
                        closureCaptureHints = {
                            enable = true,
                        },
