...
-- fixed 35-column left split via lua/jwa/explorer.lua, restoring
...
-- / close it) lives in lua/jwa/keymappings/navigation.lua with the other global
	...
	config = function()
		require("oil").setup({
			...
			keymaps = {
				...
				-- lua/jwa/explorer.lua (see the Navigation
				-- comment above for what each does where).
				["<CR>"] = {
					callback = function()
						require("jwa.explorer").select()
					end,
					desc = "Open entry (sidebar-aware)",
				},
				["<C-t>"] = {
					callback = function()
						require("jwa.explorer").smart_close()
					end,
