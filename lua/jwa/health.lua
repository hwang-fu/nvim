-- ============================================================================
-- :checkhealth jwa - the fresh-machine report (2026-08-19).
--
...
--
-- Wired by Neovim's convention: this file being lua/jwa/health.lua
-- makes `:checkhealth jwa` call M.check(). No registration needed.
-- ============================================================================
...
function M.check()
    ...
    local shim = require("jwa").phantom_shim_state()
    if shim == "ok" then
    ...
    elseif shim == "pending" then
        ...
        health.error("gitsigns.util.buf_lines is gone (plugin update?) - phantom EOF lines are disabled to protect the git index; update the shim in lua/jwa/init.lua")
    end
    ...
    for _, f in ipairs(require("jwa.lsp.format").FORMATTER_BINARIES) do
    ...
    for _, name in ipairs(require("jwa.lsp").SERVERS) do
