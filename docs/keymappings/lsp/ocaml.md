...
- Hints run near-maximal: signature codelenses appear above every `let` - top-level and nested alike - and inlay hints show parameter, let-binding, and pattern-variable types. `K` uses the extended hover, documentation and type together. If nested lenses ever read as noise, the flag is `codelens.forNestedBindings` in `lua/jwa/lsp/servers/ocamllsp.lua`.
