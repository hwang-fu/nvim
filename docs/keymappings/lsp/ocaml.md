...
- Types come from the signature codelenses above every `let` - top-level and nested alike - and from `K`, which uses the extended hover (documentation and type together). Inlay hints are deliberately off (2026-08-28): they repeated the very same types the lens line already shows, once per parameter. If nested lenses ever read as noise, the flag is `codelens.forNestedBindings` in `lua/jwa/lsp/servers/ocamllsp.lua`; the three `inlayHints.*` booleans there bring any hint kind back.
