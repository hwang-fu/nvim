...
*LSP owned by rustaceanvim (`lua/jwa/plugins/spec/rustaceanvim.lua`); server settings in `lua/jwa/lsp/servers/rust_analyzer.lua`.*
...
> [!NOTE]
> Inlay hints run near-maximal: types, parameter names, chaining types, closing braces, closure return types, elided lifetimes, binding modes, inferred generic arguments at call sites (`collect::<Vec<&str>>()` style, type and const args only), and closure capture lists. The capture list's `move(&mut count)` label reads as "what enters the closure": a reference for borrows, the bare name for real moves - the `move` word is rust-analyzer's spelling, not a claim that ownership transfers. The knobs live in `lua/jwa/lsp/servers/rust_analyzer.lua`.
