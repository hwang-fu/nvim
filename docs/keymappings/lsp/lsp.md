...
Formatting runs automatically on save for most languages; `lua/jwa/lsp/format.lua` defines which formatter each one uses. Rust and OCaml are deliberately left out and have a manual command each instead - [`:RustFmt`](rust.md) and [`:OCamlFmt`](ocaml.md) - so a save never rewrites those buffers under you.
