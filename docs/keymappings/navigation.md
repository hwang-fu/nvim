...
- `Ctrl-O` / `Ctrl-I` record every jump - `gd`, a search, `gg`/`G`, a telescope pick - so they return to the places you have actually been, including several stops inside one file. `]b` / `[b` instead cycle the buffer list in order, visited or not.
